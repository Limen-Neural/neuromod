...
2. `retune_lif_from_modulators` — recompute per-neuron `decay_rate`/`threshold` targets from the current `NeuroModulators` (dopamine/serotonin/acetylcholine/norepinephrine each pull thresholds/decay in different directions — see formulas in `engine.rs`).
3. `update_predictive_errors` — update `predictive_state` (exponential moving average (EMA) per channel) and derive `pred_errors` ("surprise") that boost synaptic drive.
4. `encode_input_spikes` — stochastically encode `stimuli` into `input_spike_times` (Poisson-style, probability proportional to stimulus magnitude). `step` draws from the thread-local RNG; `step_with_rng` draws from the caller generator and does not construct or reseed a generator per neuron or per step.
5. `integrate_lif_bank` then `fire_lif_and_inhibit` — integrate LIF membrane potentials, fire (`check_fire`), apply lateral inhibition to non-firing LIF neurons (O(1) fired-mask membership, not `Vec::contains`).
6. `apply_stdp` — reward-modulated STDP. Runs every step: per-synapse eligibility traces decay and accumulate regardless of dopamine; the `dopamine`-derived `learning_rate` gates only the trace → weight conversion.
7. `renormalize_lif_weights` — re-normalize each neuron's weights to `WEIGHT_BUDGET` (L1 budget) and clamp to the `stdp_config` bounds (`RmStdpConfig::w_min`/`w_max`).
8. `drive_izhikevich_bank` — drive the Izhikevich bank from the mean LIF membrane potential + dopamine (`iz_drive`), independent of the LIF spike/STDP pipeline.

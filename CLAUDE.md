...
`SpikingNetwork::step_with_rng` is the same pipeline with a caller-owned `&mut impl rand::Rng`. That injected random-number generator (RNG) is used only for Bernoulli encoding of `input_spike_times`. Order of work inside `step`:
...
4. Stochastically encode `stimuli` into `input_spike_times` (Poisson-style, probability proportional to stimulus magnitude). The convenience `step` draws from the thread-local RNG; `step_with_rng` draws from the caller generator and does not construct or reseed a generator per neuron or per step.

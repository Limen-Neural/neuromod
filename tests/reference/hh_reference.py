...
def rates(v):
    ...
    return (0.1 * ratio(25-v, 10), 4*math.exp(-v/18),
            .07*math.exp(-v/20), 1/(math.exp((30-v)/10)+1),
            .01*ratio(10-v, 10), .125*math.exp(-v/80))
...
def reference(cortical, current, duration, dt):
    ...
    def rhs(z):
        ...
        il=f32(.3)*(v-reversals[2])
        return (current-ina-ik-il, phi*(a*(1-m)-b*m),phi*(c*(1-h)-d*h),phi*(e*(1-n)-f*n))
    def stage(y,k,h):return tuple(a+h*b for a,b in zip(y,k))
    peak=y[0]; crossings=[]
    for i in range(round(duration/dt)):
        a=rhs(y);b=rhs(stage(y,a,dt/2));c=rhs(stage(y,b,dt/2));d=rhs(stage(y,c,dt))
        z=tuple(v+dt*(p+2*q+2*r+s)/6 for v,p,q,r,s in zip(y,a,b,c,d))
        if y[0]<threshold<=z[0]:crossings.append((i+1)*dt)
        peak=max(peak,z[0]);y=z
    return {'dt':dt,'duration':duration,'current':current,'convention':'absolute' if cortical else 'relative_to_rest','peak_voltage':peak,'crossing_times':crossings,'endpoint':list(y)}

if __name__=='__main__':
    out={}
    for name,args in [('squid_spike',(False,10,5)),('squid_subthreshold',(False,2,25)),('cortical_subthreshold',(True,20,25)),('cortical_spike',(True,500,25))]:
        out[name]=[reference(*args,dt) for dt in (.001,.0005)]
